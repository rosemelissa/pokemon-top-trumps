...
export default function PlayCommentary({
  userCards,
  setUserCards,
  compCards,
  setCompCards,
  ...
  useEffect(() => {
    if (newTurn === false) {
      if (turnNumber % 2 === 0) {
        const choices: ("hp" | "attack" | "defense" | "speed")[] = [
          "hp",
          "attack",
          "defense",
          "speed",
        ];
        const choice: "hp" | "attack" | "defense" | "speed" =
  ...
  const handleNextTurn = () => {
    ...
    if (latestWinner === "user") {
      setUserCards([...userCards, compCards[compIndex]]);
      setCompCards([
        ...compCards.slice(0, compIndex),
        ...compCards.slice(compIndex + 1),
      ]);
      setUserIndex((userIndex) => userIndex + 1);
    } else {
      setCompCards([...compCards, userCards[userIndex]]);
      setUserCards([
        ...userCards.slice(0, userIndex),
        ...userCards.slice(userIndex + 1),
      ]);
      setCompIndex((compIndex) => compIndex + 1);
    }
  };
